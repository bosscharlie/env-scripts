...
def get_free_cores(n):
  ...
  for i in range(num_window // OCCUPANCY_RATIO):
    window_usage = core_usage[i * n : i * n + n]
    # print(f"Window{i} Usage: ", window_usage)
    # 5950x only allow 1 emu
    free = sum(window_usage) < 30 * n and True not in map(lambda x: x > 80, window_usage if is_7625() else core_usage)
    if free:
      # return (Success?, memory node, start_core, end_core)
      return (True, ((i * n) % 192)//96 , i * n, i * n + n - 1)
  return (False, 0, 0, 0)
